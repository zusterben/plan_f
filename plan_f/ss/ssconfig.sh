...
create_ss_json(){
	...
	if [ "$ssconf_basic_netflix_enable" == "1" ]; then
		...
		/jffs/softcenter/bin/lua /jffs/softcenter/scripts/ss_genconfig2.lua "$ssconf_basic_node" "$NODE_MODE" 4321 1088 > $CONFIG_FILE_TMP
...
start_hysteria(){
	if [ "$(/jffs/softcenter/bin/versioncmp 4.14 $KVER)" == "1" ];then
		hysteria client --config "$CONFIG_FILE" >/dev/null 2>&1 &
	else
		QUIC_GO_DISABLE_ECN=true hysteria client --config "$CONFIG_FILE" >/dev/null 2>&1 &
	fi
	local pid
	local i=10
	until [ -n "$pid" ]; do
...
create_tuic_json() {
	...
	rm -rf "$CONFIG_FILE_TMP"
	rm -rf "$CONFIG_FILE"
	echo_date 生成tuic配置文件...
	/jffs/softcenter/bin/lua /jffs/softcenter/scripts/ss_genconfig.lua "$ssconf_basic_node" "$NODE_MODE" 3333 23456 > $CONFIG_FILE_TMP

	if [ "$(cat $CONFIG_FILE_TMP)" != "" ];then 
		echo_date 解析tuic配置文件...
		cat "$CONFIG_FILE_TMP" | jq --tab . >"$CONFIG_FILE"
		echo_date tuic配置文件写入成功到"$CONFIG_FILE"
	else
		echo_date tuic配置文件生成失败，请检查设置!!!
		rm -rf "$CONFIG_FILE_TMP"
		rm -rf "$CONFIG_FILE"
		close_in_five
	fi
...
start_tuic(){
	...
	ipt2socks -b 0.0.0.0 -4 -s 127.0.0.1 -p 23456 -l 3333 -R &
	echo_date "开启tuic-client主进程..."
	tuic-client --config "$CONFIG_FILE" &
	local pid
	local i=10
	until [ -n "$pid" ]; do
		i=$(($i - 1))
		pid=$(pidof tuic-client)
		if [ "$i" -lt 1 ]; then
			echo_date "tuic-client进程启动失败！"
			close_in_five
		fi
		usleep 250000
	done
	echo_date "tuic-client启动成功，pid：$pid"
}
write_cron_job() {

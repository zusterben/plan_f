...
local traw = io.popen("dbus get ssconf_base64_links")
...
local function processData(szType, content)
	...
	elseif szType == 'hysteria' then
		...
		result.v2ray_protocol = "hysteria"
		local dat = split(content, '%?')
		local hostInfo = split(dat[1], ':')
		result.server = hostInfo[1]
		result.server_port = hostInfo[2]
		...
		if params.obfsParam then
			result.flag_obfs = "1"
		else
			result.flag_obfs = "0"
		end
		result.salamander = params.obfsParam
		result.hy2_auth = params.auth
...
--local execute = function()
	-- exec
	do
		local raw = traw:read("*a"):gsub("\n", "")
		if #raw > 0 then
			local nodes, szType
			local groupHash = md5(raw)
			cache[groupHash] = {}
			tinsert(nodeResult, {})

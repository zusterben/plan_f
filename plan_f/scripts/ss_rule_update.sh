...
mkdir -p /tmp/upload
RULE_FILE=/jffs/softcenter/ss/rules/rules.json.js
url_main="https://raw.githubusercontent.com/zusterben/plan_f/master/rules"
...
start_update(){
	...
	rm -rf /tmp/plan_f_rule_download
	mkdir /tmp/plan_f_rule_download
	local rule_save_dir=/jffs/softcenter/ss/rules
	local rule_down_dir=/tmp/plan_f_rule_download
	local hsts=""
	...
	# 5. 先下载版本号文件
	wget -4 --no-check-certificate --timeout=8 $hsts -qO - ${url_main}/rules.json.js > /tmp/rules.json.js
	if [ "$?" == "0" ]; then
	...
	# update gfwlist
	if [ "${ssconf_basic_gfwlist_update}" == "1" ];then
		echo_date "--------------------------------------------------------------------"
		if [ "${version_gfw_local}" != "${version_gfw_online}" ];then
			...
			if [ "${md5sum_gfwlist_local}" == "${md5sum_gfw_online}" ];then
				...
				mv -f /tmp/gfwlist.conf /jffs/softcenter/ss/rules/gfwlist.conf
				run jq --arg variable "${version_gfw_online_tmp}" '.gfwlist.date = $variable' ${RULE_FILE} > tmp_${RULE_FILE}
				mv -f tmp_${RULE_FILE} ${RULE_FILE} 
				run jq --arg variable "${md5sum_gfw_online}" '.gfwlist.md5 = $variable' ${RULE_FILE} > tmp_${RULE_FILE}
				mv -f tmp_${RULE_FILE} ${RULE_FILE} 
				run jq --arg variable "${count_gfw_online}" '.gfwlist.count = $variable' ${RULE_FILE} > tmp_${RULE_FILE}
				mv -f tmp_${RULE_FILE} ${RULE_FILE} 
				reboot="1"
	...
	# update chnroute
	if [ "${ssconf_basic_chnroute_update}" == "1" ];then
		echo_date "--------------------------------------------------------------------"
		if [ "${version_chn_local}" != "${version_chn_online}" ];then
			echo_date "检测到新版本chnroute，开始更新..."
			echo_date "下载chnroute到临时文件..."
			wget -4 --no-check-certificate --timeout=8 $hsts -qO - ${url_main}/chnroute.txt > /tmp/chnroute.txt
			...
			if [ "${md5sum_chnroute_local}" == "${md5sum_chn_online}" ];then
				...
				mv -f /tmp/chnroute.txt /jffs/softcenter/ss/rules/chnroute.txt
				run jq --arg variable "${version_chn_online_tmp}" '.chnroute.date = $variable' ${RULE_FILE} > tmp_${RULE_FILE}
				mv -f tmp_${RULE_FILE} ${RULE_FILE} 
				run jq --arg variable "${md5sum_chn_online}" '.chnroute.md5 = $variable' ${RULE_FILE} > tmp_${RULE_FILE}
				mv -f tmp_${RULE_FILE} ${RULE_FILE} 
				run jq --arg variable "${count_chn_online}" '.chnroute.count = $variable' ${RULE_FILE} > tmp_${RULE_FILE}
				mv -f tmp_${RULE_FILE} ${RULE_FILE} 
				run jq --arg variable "${count_ip_chn_online}" '.chnroute.count_ip = $variable' ${RULE_FILE} > tmp_${RULE_FILE}
				mv -f tmp_${RULE_FILE} ${RULE_FILE} 
				reboot="1"
	...
	# update cdn file
	if [ "$ssconf_basic_cdn_update" == "1" ];then
		echo_date "--------------------------------------------------------------------"
		if [ "${version_cdn_local}" != "${version_cdn_online}" ];then
			echo_date "检测到新版本cdn名单，开始更新..."
			echo_date "下载cdn名单到临时文件..."
			wget -4 --no-check-certificate --timeout=8 $hsts -qO - ${url_main}/cdn.txt > /tmp/cdn.txt
			...
			if [ "${md5sum_cdn_local}" == "${md5sum_cdn_online}" ];then
				...
				mv -f /tmp/cdn.txt /jffs/softcenter/ss/rules/cdn.txt
				run jq --arg variable "${version_cdn_online_tmp}" '.cdn_china.date = $variable' ${RULE_FILE} > tmp_${RULE_FILE}
				mv -f tmp_${RULE_FILE} ${RULE_FILE} 
				run jq --arg variable "${md5sum_cdn_online}" '.cdn_china.md5 = $variable' ${RULE_FILE} > tmp_${RULE_FILE}
				mv -f tmp_${RULE_FILE} ${RULE_FILE} 
				run jq --arg variable "${count_cdn_online}" '.cdn_china.count = $variable' ${RULE_FILE} > tmp_${RULE_FILE}
				mv -f tmp_${RULE_FILE} ${RULE_FILE} 
				reboot="1"
	...
	if [ "${reboot}" == "1" ];then
		...
		run sh /jffs/softcenter/ss/ssconfig.sh restart
	fi
	echo ==================================================================================================
}
...
change_cru(){
	...
	if [ "1" == "$ssconf_basic_rule_update" ]; then
		echo_date 应用ss规则定时更新任务：每天"$ssconf_basic_rule_update_time"自动检测更新规则.
...
2)
	echo " " > /tmp/upload/ss_log.txt
	http_response "$1"
	ssconf_basic_gfwlist_update=1
	ssconf_basic_chnroute_update=1
	ssconf_basic_cdn_update=1
	change_cru > /tmp/upload/ss_log.txt

...
start_update(){
	...
	wget --no-check-certificate --timeout=8 $hsts -qO - "$url_main"/version1 > /tmp/ss_version
	if [ "$?" == "0" ]; then
	...
	if [ "$ssconf_basic_gfwlist_update" == "1" ];then
		...
		if [ ! -z "$version_gfwlist2" ];then
			if [ "$version_gfwlist1" != "$version_gfwlist2" ];then
				...
				wget --no-check-certificate --timeout=8 $hsts -qO - "$url_main"/gfwlist.conf > /tmp/gfwlist.conf
	...
	if [ "$ssconf_basic_chnroute_update" == "1" ];then
		...
		if [ ! -z "$version_chnroute2" ];then
			if [ "$version_chnroute1" != "$version_chnroute2" ];then
				...
				wget --no-check-certificate --timeout=8 $hsts -qO - "$url_main"/chnroute.txt > /tmp/chnroute.txt
	...
	if [ "$ssconf_basic_cdn_update" == "1" ];then
		...
		if [ ! -z "$version_cdn2" ];then
			if [ "$version_cdn1" != "$version_cdn2" ];then
				...
				wget --no-check-certificate --timeout=8 $hsts -qO - "$url_main"/cdn.txt > /tmp/cdn.txt

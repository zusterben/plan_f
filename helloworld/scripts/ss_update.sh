...
main_url="https://raw.githubusercontent.com/zusterben/plan_f/master/bin/$ARCH_SUFFIX"
backup_url=""
...
update_ss(){
	...
	if [ -n "$ss_basic_version_web1" ];then
		...
		if [ "$ssconf_basic_version_local" != "$ss_basic_version_web1" ];then
		echo_date 主服务器在线版本号："$ss_basic_version_web1" 和本地版本号："$ssconf_basic_version_local" 不同！
			cd /tmp
			...
			echo_date 主服务器在线版本号："$ss_basic_version_web1" 和本地版本号："$ssconf_basic_version_local" 相同！
			echo_date 退出插件更新!
			sleep 1
			echo XU6J03M6
			exit
...
update_ss2(){
	...
	echo_date "历史版本下载地址：https://raw.githubusercontent.com/zusterben/plan_f/master/history_package/$ARCH_SUFFIX/"

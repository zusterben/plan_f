...
function confdecode(obj) {
	for (var i = 1; obj["ssconf_basic_json_" + i]; i++){
...
function conf2obj(obj, p) {
	...
	if(type == "shadowsocks"){
		...
		if (transport == "tcp") {
			...
			E(p + "v2_uplink_capacity").value = c.uplink_capacity || "5";
			E(p + "v2_downlink_capacity").value = c.downlink_capacity || "20";
...
function save() {
	...
	if(node_type == 0){
		...
		if (tmp_db.transport == "tcp") {
			...
			tmp_db.uplink_capacity = E("sstable_v2_uplink_capacity").value;
			tmp_db.downlink_capacity = E("sstable_v2_downlink_capacity").value;
...
function verifyFields(r) {
	...
	elem.display(elem.parentElem('sstable_insecure', 'tr'), (v2ray_on && (vmess_on || vless_on)));
	...
	elem.display(elem.parentElem('sstable_v2_uplink_capacity', 'tr'), kcp_on);
	elem.display(elem.parentElem('sstable_v2_downlink_capacity', 'tr'), kcp_on);
	...
	if (save_flag == "v2ray") {
		...
		showhide("ss_node_table_v2_downlink_capacity_tr", E("ss_node_table_v2_transport").value == "mkcp");
...
function add_edit_node(flag, node_idx, add) {
	...
	if(flag == 'shadowsocks'){
		...
		if (tmp_db.transport == "tcp") {
			...
			tmp_db.uplink_capacity = $.trim($('#ss_node_table_v2_uplink_capacity').val());
			tmp_db.downlink_capacity = $.trim($('#ss_node_table_v2_downlink_capacity').val());
...
function version_show() {
	...
	$.ajax({
		url: 'https://raw.githubusercontent.com/zusterben/plan_f/master/bin/' + scarch + '/config.json.js',
		type: 'GET',
		dataType: 'json',
												...
												<tr style="height:32px;">
													<td>
														<table width="100%" border="0" align="left" cellpadding="0" cellspacing="0" class="vpnClientTitle">
															<tr>
															<td width="25%" align="center" id="ssTitle" onclick="tabclickhandler(0);">添加SS配置</td>
															<td width="25%" align="center" id="ssrTitle" onclick="tabclickhandler(1);">添加SSR配置</td>
														...
														<table id="table_edit" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable">
															<script type="text/javascript">
																$('#table_edit').forms([
																	...
																	{ title: '上行链路容量', rid:'ss_node_table_v2_uplink_capacity_tr', id:'ss_node_table_v2_uplink_capacity', type:'text', maxlen:'50', hidden:"yes", value: "5"},
																	{ title: '下行链路容量', rid:'ss_node_table_v2_downlink_capacity_tr', id:'ss_node_table_v2_downlink_capacity', type:'text', maxlen:'50', hidden:"yes", value: "20"},
											...
											<table id="table_basic" width="100%" border="0" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
												<script type="text/javascript">
													$('#table_basic').forms([
														...
														{ title: '上行链路容量', id:'sstable_v2_uplink_capacity', type:'text', maxlen:'50', hidden:"yes", value: "5"},
														{ title: '下行链路容量', id:'sstable_v2_downlink_capacity', type:'text', maxlen:'50', hidden:"yes", value: "20"},
										...
										<div id="tablet_3" style="display: none;">
											<table id="table_dns" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
												<script type="text/javascript">
													...
													var option_dnsf = [ ["1", "pdnsd"], ["2", "chinadns-ng"], ["3", "dns2socks"], ["8", "direct"]];
